...
<script>
    ...
    function initDropzone() {
        ...
        $("#uploaded").dropzone({
            ...
            accept: function(file, done) {
                if (total_files <= 9) {
                    if (file.type != "image/jpeg" && file.type != "image/svg" && file.type !=
                        "image/jpg" &&
                        file.type != "image/png" &&
                        file
                        ...
                        file.type != "video/mp4") {
                        toastr.error(
                            "Please upload a file of type jpeg/jpg/svg/png/gif/pdf/zip/mp4",
                            "Error!");
                    } else {
                        done();
                    }
     ...
     */
    function applyWaterMarkImage(file_id) {
        "use strict";
        $.post("{{ route('core.apply.watermark.image') }}", {
                _token: '{{ csrf_token() }}',
                file_id: file_id,
            },

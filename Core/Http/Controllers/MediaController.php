...
class MediaController extends Controller
{
    ...
    {
        $water_mark_applicable_sellectd_flders = DB::table('tl_water_mark_image_applicable_folders')
            ->pluck('image_type')->toArray();
        ...
        $data['water_mark_applicable_sellectd_flders'] = $water_mark_applicable_sellectd_flders;
    ...
    public function storeMediaSettings(MediaRequest $request)
    {
        try {
            ...
            if (isset($request['image_applicable_folder'])) {
                $water_applicable_sellectd_flders = $all_request['image_applicable_folder'];

                DB::table('tl_water_mark_image_applicable_folders')->delete();

                $data = [];

                for ($i = 0; $i < sizeof($water_applicable_sellectd_flders); $i++) {
                    array_push($data, ['image_type' => $water_applicable_sellectd_flders[$i]]);
                }

                WatermarkApplicableFolder::insert($data);
            }
            $data = [];

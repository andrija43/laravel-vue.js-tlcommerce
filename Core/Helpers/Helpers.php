...
if (!function_exists('saveFileInStorage')) {
    ...
    function saveFileInStorage($file, $file_name, $location, $file_type, $media_type)
    {
        ...
        if (File::exists('public/storage/' . $location)) {
        ...
        $image_name = $db_file->name;
        $folder_name = $db_file->folder_name;
        $extension = explode('.', $db_file->path)[1];
        ...
        if ($file_type == 'image') {
            ...
            if ($is_watermark_enable == 'on') {
                $data = [
                    ...
                    'extension' => $extension,
                    'width' => $cropping_sizes['large_size'][0],
                    'height' => $cropping_sizes['large_size'][1]
                ];
                $watermark_jobs = new ApplyWaterMarkImage($data);
                dispatch($watermark_jobs);
                $data = [
                    'image_name' => $image_name,
                    'folder_name' => $folder_name,
                    'extension' => $extension,
                    'width' => $cropping_sizes['medium_size'][0],
                    'height' => $cropping_sizes['medium_size'][1]
                ];
                $watermark_jobs = new ApplyWaterMarkImage($data);
                dispatch($watermark_jobs);
                ...
                $data = [
                    ...
                    'folder_name' => $folder_name,
                    'extension' => $extension,
                    'width' => $cropping_sizes['small_size'][0],
                    'height' => $cropping_sizes['small_size'][1]
                ];
                $watermark_jobs = new ApplyWaterMarkImage($data);
                dispatch($watermark_jobs);
            }
...
if (!function_exists('getDefaultCroppingSizes')) {
    ...
    function getDefaultCroppingSizes($file, $location, $original_path, $original_file_name)
    {
        ...
        $img = Image::make('public/storage/' . $original_path)->resize($large_thumb_image[0], $large_thumb_image[1]);
        ...
        $img = Image::make('public/storage/' . $original_path)->resize($medium_thumb_image[0], $medium_thumb_image[1]);
        $img->save('public/storage/' . $path);
        chmod('public/storage/' . $path, 0777);
        ...
        $img = Image::make('public/storage/' . $original_path)->resize($small_thumb_image[0], $small_thumb_image[1]);
        $img->save('public/storage/' . $path);
        chmod('public/storage/' . $path, 0777);
...
if (!function_exists('applyWaterMarkImage')) {
     ...
     * @return mixed
     */
    function applyWaterMarkImage($image_name, $folder_name, $extension, $width, $height)
    {
        $path = $folder_name . '/' . $image_name . $width . 'x' . $height . '.' . $extension;
        ...
        if ($watermark_image_location_object != null) {
            $watermark_image_location = $watermark_image_location_object->value;
        }

        if ($watermark_image_location != null && $watermark_image_id != null && $watermark_image_position != null && $watermark_image_opacity != null) {
            $watermark = Image::make('public/' . $watermark_image_location)->resize($width, $height);
